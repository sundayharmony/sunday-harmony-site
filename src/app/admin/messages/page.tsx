  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-[280px_1fr] gap-4 h-[calc(100vh-12rem)]">
        {/* Client List */}
        <div className="bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.06)] rounded-xl overflow-y-auto">
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-lg mb-1 transition-all ${
                      isSelected
                        ? 'bg-[rgba(201,169,110,0.1)] border border-[rgba(201,169,110,0.2)]'
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      {msgCount > 0 && (
                        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-[rgba(201,169,110,0.15)] text-brand-gold">
                          {msgCount}
                        </span>
                      )}
                    </div>
        ...
        {/* Chat Area */}
        <div className="flex flex-col bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.06)] rounded-xl overflow-hidden">
          {selectedClient ? (
            <>
              {/* Chat Header */}
              <div className="px-5 py-3 border-b border-[rgba(255,255,255,0.06)] flex items-center gap-3">
                <div>
                      ...
                      <div key={msg.id} className={`flex ${isAdmin ? 'justify-end' : 'justify-start'}`}>
                        <div
                          className={`max-w-[70%] rounded-2xl p-4 ${
                            isAdmin
                              ? 'bg-[rgba(201,169,110,0.1)] border border-[rgba(201,169,110,0.2)]'
                              : 'bg-[rgba(74,158,125,0.1)] border border-[rgba(74,158,125,0.2)]'
                          }`}
                        >
              ...
              {/* Reply Input */}
              <form onSubmit={sendMessage} className="p-3 border-t border-[rgba(255,255,255,0.06)] flex gap-3">
                <input
                  type="text"
                  value={newMsg}
                  onChange={e => setNewMsg(e.target.value)}
                  placeholder={`Reply to ${selectedClientData?.name?.split(' ')[0] || 'client'}...`}
                  className="flex-1 py-2.5 px-4 bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.06)] rounded-xl text-brand-text text-sm outline-none focus:border-[rgba(201,169,110,0.3)] transition-colors"
                />
                <button
                  type="submit"
                  disabled={!newMsg.trim() || sending}
                  className="px-5 py-2.5 rounded-xl bg-gradient-to-br from-brand-gold to-[#b8944f] text-[#0a0a0f] text-sm font-bold hover:-translate-y-0.5 transition-all disabled:opacity-50 disabled:hover:translate-y-0"
                >
