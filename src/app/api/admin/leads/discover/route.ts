...
export async function POST(req: NextRequest) {
  ...
  const service = asText(body?.service).trim()
  const city = asText(body?.city).trim()
  ...
  const response = await fetch('https://places.googleapis.com/v1/places:searchText', {
    ...
    headers: {
      ...
      'X-Goog-FieldMask': 'places.id,places.displayName,places.formattedAddress,places.internationalPhoneNumber,places.websiteUri,places.rating,places.userRatingCount,places.businessStatus',
    },
    body: JSON.stringify({
      textQuery,
      pageSize: maxResults,
  ...
  if (!response.ok) {
    ...
    return NextResponse.json({ error: 'Failed to fetch businesses from Google Places' }, { status: 502 })
  }
